...
drawbar(Monitor *m)
{
    ...
    if ((w = m->ww - tw - stw - x) > bh) {
        if (m->sel) {
            drw_text (drw, x, -1, w, bh, 0, m->sel->name, 0);
        } else {
    ...
    for (i = 0; i < LENGTH (tags); i++) {
        if (occ & 1 << i)
            drw_rect (drw, x + boxs - 2, boxs - 2, 2 * boxw, 2 * boxw, 1, 0);
        k = (m->tagset[m->seltags] & 1 << i) ? tag_ratio : 1.0;
        drw_rounded_rect (drw, x + boxw, boxw - 1, k * bh - boxw * 2, bh - boxw * 2, ( k > 1) && (m == selmon), 0);
        x += k * bh;
    }
...
unsigned int nexttag (void) {
    ...
    if (!c) return seltag;
    do { usedtags |= c->tags; c = c->next; } while (c);
    do { seltag = seltag == (1 << (LENGTH (tags) - 1)) ? 1 : seltag << 1; } while (!(seltag & usedtags));
    return seltag;
}
